...
import { useParentRescheduleRequests } from "@/features/parent/hooks/useRescheduleRequests";
 ...
 * 05-MODULE-SPECS-INDEX.md). Requests awaiting the Parent's own
 * response (Teacher-proposed) show first; everything else (raised by
 * the Parent themselves, or already resolved) follows as history.
 * Propose a new reschedule from a specific class's own page instead
 * — /parent/classes/[sessionId]/reschedule.
 */
export default function ParentReschedulePage() {
  const { requests, loading, error, actingId, approve, reject, cancel } =
    useParentRescheduleRequests();

  const needsResponse = requests.filter((r) => isPendingOnViewer(r, "PARENT"));
  const others = requests.filter((r) => !isPendingOnViewer(r, "PARENT"));
