...
/** Teacher-side reschedule-request inbox: load, approve/reject, or withdraw. */
export function useTeacherRescheduleRequests() {
  ...
  async function act(requestId: string, action: "approve" | "reject" | "cancel", responseNote?: string) {
    try {
      ...
      const res = await fetch(`/api/teacher/reschedule-requests/${requestId}`, {
        method: "PATCH",
